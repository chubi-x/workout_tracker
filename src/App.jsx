...
import { exercises, workouts, adHocExercises } from "./data.js";
import {
  appendSession,
  elapsedMilliseconds,
  formatDuration,
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <XAxis
              ...
              minTickGap={32}
              tickLine={false}
              axisLine={{ stroke: "rgb(21 21 18 / 35%)" }}
            />
            <YAxis
              domain={[0, "auto"]}
      ...
      <p className="chart-selection" aria-live="polite">
        {selectedPoint ? (
          <>
            ...
            <strong>{formatValue(selectedPoint.value)}</strong>
          </>
        ) : (
...
function ProgressView({ sessions, chooseView }) {
  ...
  if (!sessions.length) {
    return (
      <EmptyView title="No progress data." chooseView={chooseView}>
        Complete a workout to start your progress report.
      </EmptyView>
    );
  }
    ...
    <main className="report-view">
      ...
      <section className="metrics" aria-label="Progress summary">
        ...
        <div>
          ...
          <strong>{workoutById.get(latest.workoutId)?.label ?? "—"}</strong>
...
function LogView({ sessions, chooseView, deleteSession }) {
  if (!sessions.length) {
    return (
      <EmptyView title="The log is empty." chooseView={chooseView}>
        Complete a workout to make your first entry.
      </EmptyView>
    );
  }
  ...
  return (
    <main className="log-view">
      <header className="log-header">
        <p className="eyebrow">Chronological record / Newest first</p>
        <h1>
          Training
          <br />
          <em>log.</em>
              ...
              <article className="log-entry">
                <header>
                  ...
                  <div>
                    ...
                    <h2>
                      {workout
                        ? `Workout ${workout.label} / ${workout.name}`
                        : session.workoutId}
                    </h2>
                  </div>
                ...
                <details>
                  <summary>
                    View {session.exercises.length} logged{" "}
                  ...
                  <div className="logged-exercises">
                    {session.exercises.map((log) => {
                      ...
                      return (
                        <section key={log.exerciseId}>
                          <h3>{exercise?.name ?? log.exerciseId}</h3>
                          <ol>
                              ...
                              <li key={setIndex}>
                                ...
                                <strong>
                                  {exercise?.mode === "duration"
                                    ? formatDuration(set.durationSeconds * 1000)
                                    : `${set.reps} reps${set.weightKg === undefined ? "" : ` / ${set.weightKg} kg`}`}
                                </strong>
                              </li>
                            ))}
                          </ol>
...
function RepsLogger({ log, updateLog }) {
  const updateSet = (index, field, value) => {
    const sets = log.sets.map((set, setIndex) =>
    ...
    <div className="set-logger">
          ...
          <fieldset className="set-row" key={index}>
            ...
            <label>
              ...
              <input
                ...
                value={set.reps}
                onInput={(event) =>
                  updateSet(index, "reps", event.currentTarget.value)
                }
              />
            </label>
            <label>
              ...
              <input
                ...
                value={set.weightKg}
                onInput={(event) =>
                  updateSet(index, "weightKg", event.currentTarget.value)
                }
              />
            </label>
            <button
      ...
      <button
        ...
        onClick={() =>
          updateLog({
            ...log,
            sets: [...log.sets, { reps: "", weightKg: DEFAULT_WEIGHT_KG }],
          })
        }
      >
        + Add set
...
function DurationLogger({ exercise, log, updateLog, now }) {
  ...
  const readySeconds = running
    ? Math.max(0, Math.ceil((log.timer.startedAt - now) / 1000))
    : 0;
    ...
    <div className="duration-logger">
      <label className="target-input">
        ...
        <input
          ...
          step="1"
          required
          value={log.targetDuration}
          onInput={(event) =>
            updateLog({ ...log, targetDuration: event.currentTarget.value })
          }
        />
      </label>
      <div className="timer-readout" role="timer" aria-live="off">
        {preparing ? (
          ...
          <>
            ...
            <span>
              / {formatDuration(validTarget ? targetDuration * 1000 : 0)}
            </span>
          </>
        )}
      </div>
      <div
        ...
        aria-label={`${exercise.name} timer controls`}
      >
        <button
          type="button"
          onClick={() =>
            updateLog({
              ...log,
              timer: { elapsedMs: 0, startedAt: Date.now() + TIMER_BUFFER_MS },
            })
          }
          disabled={active || !validTarget}
        >
          Start
        </button>
        <button
        ...
        <button type="button" onClick={stop} disabled={!active || preparing}>
          Stop
        </button>
        <button
          type="button"
          onClick={() =>
            updateLog({ ...log, timer: { elapsedMs: 0, startedAt: null } })
          }
          disabled={!active}
        >
          Reset
      ...
      <ol className="duration-sets" aria-label="Completed duration sets">
        {log.sets.map((set, index) => (
          <li key={index}>
            <span>
              Set {index + 1}: {formatDuration(set.durationSeconds * 1000)}
            </span>
            <button
              className="text-button"
              type="button"
      ...
      {log.sets.length === 0 && (
        <p className="empty-note">Stop the timer to log a set.</p>
      )}
    </div>
  );
}
    ...
    <main className="app-home">
      ...
      <section className="workout-overview" aria-label="Workout overview">
        {workouts.map((workout) => {
          const workoutExercises = workout.exerciseIds.map((id) =>
            exerciseById.get(id),
          );
            ...
            <article className="workout-card" key={workout.id}>
              ...
              <div className="workout-card-copy">
                <div className="workout-card-title">
                  <span>Workout {workout.label}</span>
                  <strong>{workout.exerciseIds.length} exercises</strong>
                </div>
                <h2>{workout.name}</h2>
                <p>{workout.note}</p>
              ...
              <div className="workout-card-actions">
                ...
                <button
                  ...
                  onClick={() =>
                    isActive ? openWorkout(workout.id) : startWorkout(workout)
                  }
                >
...
function WorkoutDetailView({
  ...
  const isActive = activeSession?.workoutId === workout.id;
  const activeExerciseIds = isActive
    ? Object.keys(activeSession.exercises)
    : workout.exerciseIds;
  const availableAdHoc = adHocExercises.filter(
    ({ id }) => !activeExerciseIds.includes(id),
  );
  ...
  return (
    <main className="workout-detail">
      ...
      <header className="detail-header">
        <div>
          <p className="eyebrow">Workout {workout.label}</p>
          <h1>{workout.name}</h1>
        </div>
        <div className="detail-summary">
          <strong>{activeExerciseIds.length} exercises</strong>
          <p>{workout.note}</p>
        </div>
      </header>
      ...
      {isActive ? (
        <section className="session-bar" aria-label="Active workout">
          <div>
            ...
            <h2>Workout {workout.label}</h2>
          </div>
          <div className="overall-timer">
            <span>Elapsed</span>
            <strong>
              {formatDuration(Math.max(0, now - activeSession.startedAt))}
            </strong>
          </div>
          ...
          {!canFinish && (
            <p className="finish-note">
              {hasActiveTimer
                ? "Stop or reset each exercise timer before you finish."
                : "Log at least one valid set to finish."}
            </p>
          )}
        </section>
      ) : activeSession ? (
        <button
          ...
          type="button"
          onClick={() => openWorkout(activeSession.workoutId)}
        >
          ...
          <div className="session-actions">
            {availableAdHoc.map((exercise) => (
              <button
                ...
                type="button"
                key={exercise.id}
                onClick={() => addAdHocExercise(exercise.id)}
              >
                + {exercise.name}
              </button>
            ))}
      ...
      <section
        className="detail-exercises"
        aria-labelledby={`${workout.id}-exercises`}
      >
...
function restoreSession() {
  ...
  const workout =
    session && workouts.find(({ id }) => id === session.workoutId);
...
function App() {
  ...
  const repLogsValid =
    activeSession &&
    Object.entries(activeSession.exercises).every(
      ([id, log]) =>
        exerciseById.get(id).mode !== "reps" ||
        log.sets.every(validOrEmptyRepsSet),
    );
  const hasSets =
    activeSession &&
    Object.entries(activeSession.exercises).some(([id, log]) =>
      exerciseById.get(id).mode === "reps"
        ? log.sets.some(validRepsSet)
        : log.sets.length > 0,
    );
  const hasActiveTimer =
    activeSession &&
    Object.values(activeSession.exercises).some(
      ({ timer }) => timer && (timer.startedAt !== null || timer.elapsedMs > 0),
    );
  ...
  const updateExercise = (id, log) => {
    setActiveSession((session) => ({
      ...session,
      exercises: { ...session.exercises, [id]: log },
    }));
  };

  const addAdHocExercise = (id) => {
    const exercise = exerciseById.get(id);
    if (!exercise || !adHocExercises.some(({ id: adHocId }) => adHocId === id))
      return;
    setActiveSession((session) => {
  ...
  const finishSession = () => {
    ...
    const completed = {
      ...
      exercises: Object.entries(activeSession.exercises).flatMap(
        ([exerciseId, log]) => {
          if (log.sets.length === 0) return [];
          const exercise = exerciseById.get(exerciseId);
          const sets =
            exercise.mode === "reps"
              ? completedRepsSets(log.sets).map((set) => ({
  ...
  const cancelSession = () => {
    if (!window.confirm("Cancel this workout and discard all logged sets?"))
      return;
    setActiveSession(null);
  ...
  const deleteSession = (session) => {
    if (
      !window.confirm(
        `Delete the ${formatDate(session.startedAt)} workout? This cannot be undone.`,
      )
    )
      return;
